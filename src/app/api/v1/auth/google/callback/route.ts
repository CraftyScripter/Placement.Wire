...
import { recordFailedLogin, recordVisit } from '@/lib/analytics/store';
...
export async function GET(req: NextRequest) {
  ...
  if (oauthError) {
    ...
    await recordFailedLogin({ reason: `oauth_error:${oauthError}` });
    return NextResponse.redirect(`${baseUrl}/login?error=${encodeURIComponent(oauthError)}`);
  }

  if (!code || !state) {
    await recordFailedLogin({ reason: 'missing_code_or_state' });
  ...
  if (!isStateValid) {
    ...
    await recordFailedLogin({ reason: 'invalid_csrf_state' });
  ...
  try {
    ...
    if (!profile.verified_email) {
      await recordFailedLogin({ reason: 'unverified_email', email: profile.email || null });
    ...
    if (!validation.isValid) {
      ...
      await recordFailedLogin({ reason: 'domain_unauthorized', email: profile.email || null });
      return NextResponse.redirect(

  ...
  return (
    <div className="min-h-screen bg-canvas text-[#323243]">
      ...
      <main className="mx-auto max-w-4xl px-4 pb-16 pt-6 sm:px-6 sm:pt-10">
        <p className="text-xs font-normal uppercase tracking-wider text-primary">About</p>
        <h1 className="mt-2 text-3xl font-semibold tracking-tight text-[#323243] sm:text-4xl">
          Placement emails, finally organized.
        </h1>
        <p className="mt-4 max-w-2xl text-sm font-normal leading-relaxed text-muted sm:text-base">
        ...
        <div className="mt-8 grid grid-cols-1 gap-3 sm:gap-4 min-[560px]:grid-cols-2 md:grid-cols-3">
          {HIGHLIGHTS.map((h) => (
            <div key={h.title} className="rounded-lg border border-line bg-white p-4 shadow-card">
              <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-md border border-line bg-canvas text-primary">
                <h.icon className="h-4 w-4" />
              </div>
              <h2 className="text-sm font-semibold text-[#323243]">{h.title}</h2>
              <p className="mt-1.5 text-xs font-normal leading-relaxed text-muted">{h.text}</p>
            </div>
          ))}
        </div>

...
export default function AdminLoginsPage() {
  ...
  const totalShownLogins = perDay.reduce((sum, d) => sum + d.logins, 0);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Full-history Logins Trend Chart */}
      <Card>
        <CardTitle
          icon={TrendingUp}
          subtitle="Daily authentications — full history, nothing ever deleted"
          right={
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-[#94A3B8]">
              <span>Total shown:</span>
              <strong className="text-slate-900 dark:text-white">{totalShownLogins}</strong>
            </div>
          }
        >

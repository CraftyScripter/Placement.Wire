...
export default function AdminSignupsPage() {
  ...
  const totalShownSignups = perDay.reduce((sum, d) => sum + d.signups, 0);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Full-history Signups Trend Chart */}
      <Card>
        <CardTitle
          icon={TrendingUp}
          subtitle="Daily account registrations — full history, nothing ever deleted"
          right={
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-[#94A3B8]">
              <span>Total shown:</span>
              <strong className="text-slate-900 dark:text-white">{totalShownSignups}</strong>
            </div>
          }
        >
          Registration Trend

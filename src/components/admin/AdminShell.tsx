    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#0B0E14] dark:text-[#E2E4ED] transition-colors">
      <div className="mx-auto flex w-full max-w-[1720px] flex-col gap-4 p-3 sm:p-5 lg:flex-row lg:gap-6">
        ...
        <aside className="hidden w-[260px] shrink-0 lg:block xl:w-[280px]">
          <div className="sticky top-5 flex h-[calc(100vh-2.5rem)] flex-col rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm dark:border-[#1F2430] dark:bg-[#141824] dark:shadow-none">
            {/* Header Brand */}
        ...
        <main className="min-w-0 flex-1 [&_thead_th]:sticky [&_thead_th]:top-0 [&_thead_th]:z-10 [&_thead_th]:bg-slate-50 dark:[&_thead_th]:bg-[#141824] [&_thead_th]:border-b [&_thead_th]:border-slate-200 dark:[&_thead_th]:border-[#1F2430] [&_thead_th]:text-slate-500 dark:[&_thead_th]:text-[#94A3B8]">
          {/* Storage health — tells the truth about WHERE this panel's data lives.
              Localhost shows localhost hits only; production data lives where
              production runs. Mode 'none' means hits are being dropped. */}
          {data?.storage && (
            <div
              className={`mb-4 flex items-start gap-2.5 rounded-xl border px-4 py-3 text-xs ${
                data.storage.mode === 'none'
                  ? 'border-error/40 bg-error-soft text-error'
                  : 'border-line bg-white text-muted dark:border-white/10 dark:bg-[#141824] dark:text-[#94A3B8]'
              }`}
              role={data.storage.mode === 'none' ? 'alert' : 'status'}
            >
              <span
                className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${
                  data.storage.mode === 'none'
                    ? 'bg-error'
                    : data.storage.mode === 'drive'
                      ? 'bg-sky-500'
                      : 'bg-success'
                }`}
              />
              <div>
                <p className="flex flex-wrap items-center gap-2 font-semibold text-slate-800 dark:text-[#E2E4ED]">
                  <span>
                    {data.storage.mode === 'none'
                      ? 'Analytics OFF — visits and signups are being dropped'
                      : data.storage.mode === 'drive'
                        ? 'Reading from Google Drive (serverless backend)'
                  ...
                  <span
                    ...
                    title={
                      data.environment === 'production'
                        ? 'PRODUCTION data — real users. Localhost hits never appear here.'
                        : 'LOCAL DEV data — your own testing only. Production users appear in the deployed admin.'
                    }
                  >
                    {data.environment === 'production' ? '● Production' : '● Local dev'}
                  </span>
                ...
                <p className="mt-0.5 leading-relaxed">
                  {data.storage.mode === 'none'
                    ? 'Set ADMIN_DRIVE_REFRESH_TOKEN (Admin → Backup → Drive logging) or deploy where disk is writable. Until then, nobody is being counted.'
                    : 'This panel shows hits recorded by THIS server only. Users on other deployments appear in that deployment\u2019s admin, not here.'}
                </p>
              </div>
            </div>
          )}
